...
import { STATUS } from "../services/candidaturaService.js";
...
const metricProjetos = document.getElementById("metricProjetos");
const metricPropostas = document.getElementById("metricPropostas");
const metricContratacoes = document.getElementById("metricContratacoes");
const empresaProfileName = document.getElementById("empresaProfileName");
const empresaPanelName = document.getElementById("empresaPanelName");
...
function renderDashboardResumo() {
  const nomeEmpresa = state.profile?.nome || "Empresa BMO";
  const inicial = nomeEmpresa.trim().charAt(0).toUpperCase() || "E";
  const contratacoes = state.candidaturas.filter((item) => item.candidatura.status === STATUS.ACEITO).length;

  if (metricProjetos) metricProjetos.textContent = state.problemas.length;
  if (metricPropostas) metricPropostas.textContent = state.candidaturas.length;
  if (metricContratacoes) metricContratacoes.textContent = contratacoes;
  [empresaProfileName, empresaPanelName].forEach((element) => {
    if (element) element.textContent = nomeEmpresa;
  });
